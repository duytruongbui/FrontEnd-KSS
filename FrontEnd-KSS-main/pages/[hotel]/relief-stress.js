...
import { AllQuestions2, MantraList2 } from '../../components/quiz/quiz1data';
...
const quiz = ({ Q1List, Q2List, Q3List }) => {
  ...
  const onSubmit = () => {
    ...
    resSorted.map((el, index) => {
      el == Math.max(...resSorted) ? mantraList2.push(index) : null;
    });
    let res = [];
    let resultTieCheck = false;
    let changeShowCheck = false;
    if (mantraList2.length === 1) {
      onChangeMantra(mantraList2[0]);
      resultTieCheck = false;
      onShowMantra(true, MantraList2[mantraList2[0]]);
    } else if (mantraList2.length <= 3) {
      mantraList2.map((mantraEl) => {
        let checkStop = true;
        let i = 0;
        while (checkStop) {
  ...
  const onSecondSubmit = (anss, newDragList) => {
    ...
    if (selectedMantra > 0) {
      ...
      onShowMantra(true, MantraList2[selectedMantra]);
      onResultTie(false);
    } else {
  ...
  const showResultMantra = (Mantra) => {
    ...
    onShowMantra(true, MantraList2[Mantra[0].ind]);
    console.log(mantra);
  };
    ...
    try {
      const res = await api.quiz.submitAnswerText({
        questionOptionsTextList: ansList
          ...
          .map((as) => as.data),
        topic: AllQuestions2[curPage].question,
                  ...
                  {resultTie ? (
                    ...
                    <div className="mobile:w-full">
                      ...
                      <div className={!showItem ? 'block' : 'hidden'}>
                        {AllQuestions2.map((content, index) => (
                          <div
