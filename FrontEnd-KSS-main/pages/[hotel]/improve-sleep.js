...
import { AllQuestions3, MantraList3 } from '../../components/quiz/quiz1data';
...
const quiz = ({ Q1List, Q2List, Q3List }) => {
  ...
  const onSubmit = () => {
    ...
    resSorted.map((el, index) => {
      el == Math.max(...resSorted) ? mantraList3.push(index) : null;
    });
    let res = [];
    let resultTieCheck = false;
    let changeShowCheck = false;
    if (mantraList3.length === 1) {
      onChangeMantra(mantraList3[0]);
      resultTieCheck = false;
      onShowMantra(true, MantraList3[mantraList3[0]]);
    } else if (mantraList3.length <= 3) {
      mantraList3.map((mantraEl) => {
        let checkStop = true;
        let i = 0;
        while (checkStop) {
  ...
  const onSecondSubmit = (anss, newDragList) => {
    ...
    if (selectedMantra > 0) {
      ...
      onShowMantra(true, MantraList3[selectedMantra]);
      onResultTie(false);
    } else {
  ...
  const showResultMantra = (Mantra) => {
    ...
    onShowMantra(true, MantraList3[Mantra[0].ind]);
    console.log(mantra);
  };
    ...
    try {
      const res = await api.quiz.submitAnswerText({
        questionOptionsTextList: ansList
          ...
          .map((as) => as.data),
        topic: AllQuestions3[curPage].question,
                  ...
                  {resultTie ? (
                    ...
                    <div className="mobile:w-full">
                      ...
                      <div className={!showItem ? 'block' : 'hidden'}>
                        {AllQuestions3.map((content, index) => (
                          <div
