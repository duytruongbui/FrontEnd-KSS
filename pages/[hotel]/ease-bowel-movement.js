...
import { AllQuestions1, MantraList1 } from '../../components/quiz/quiz1data';
...
const quiz = ({ Q1List, Q2List, Q3List }) => {
  ...
  const onSubmit = () => {
    ...
    resSorted.map((el, index) => {
      el == Math.max(...resSorted) ? mantraList1.push(index) : null;
    });
    let res = [];
    let resultTieCheck = false;
    let changeShowCheck = false;
    if (mantraList1.length === 1) {
      onChangeMantra(mantraList1[0]);
      resultTieCheck = false;
      onShowMantra(true, MantraList1[mantraList1[0]]);
    } else if (mantraList1.length <= 3) {
      mantraList1.map((mantraEl) => {
        let checkStop = true;
        let i = 0;
        while (checkStop) {
  ...
  const onSecondSubmit = (anss, newDragList) => {
    ...
    if (selectedMantra > 0) {
      ...
      onShowMantra(true, MantraList1[selectedMantra]);
      onResultTie(false);
    } else {
  ...
  const showResultMantra = (Mantra) => {
    ...
    onShowMantra(true, MantraList1[Mantra[0].ind]);
    console.log(mantra);
  };
    ...
    try {
      const res = await api.quiz.submitAnswerText({
        questionOptionsTextList: ansList
          ...
          .map((as) => as.data),
        topic: AllQuestions1[curPage].question,
                  ...
                  {resultTie ? (
                    ...
                    <div className="mobile:w-full">
                      ...
                      <div className={!showItem ? 'block' : 'hidden'}>
                        {AllQuestions1.map((content, index) => (
                          <div
