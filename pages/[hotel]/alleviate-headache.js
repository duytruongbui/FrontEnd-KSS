...
import { AllQuestions4, MantraList4 } from '../../components/quiz/quiz1data';
...
const quiz = ({ Q1List, Q2List, Q3List }) => {
  ...
  const onSubmit = () => {
    ...
    resSorted.map((el, index) => {
      el == Math.max(...resSorted) ? mantraList4.push(index) : null;
    });
    let res = [];
    let resultTieCheck = false;
    let changeShowCheck = false;
    if (mantraList4.length === 1) {
      onChangeMantra(mantraList4[0]);
      resultTieCheck = false;
      onShowMantra(true, MantraList4[mantraList4[0]]);
    } else if (mantraList4.length <= 3) {
      mantraList4.map((mantraEl) => {
        let checkStop = true;
        let i = 0;
        while (checkStop) {
  ...
  const onSecondSubmit = (anss, newDragList) => {
    ...
    if (selectedMantra > 0) {
      ...
      onShowMantra(true, MantraList4[selectedMantra]);
      onResultTie(false);
    } else {
  ...
  const showResultMantra = (Mantra) => {
    ...
    onShowMantra(true, MantraList4[Mantra[0].ind]);
    console.log(mantra);
  };
    ...
    try {
      const res = await api.quiz.submitAnswerText({
        questionOptionsTextList: ansList
          ...
          .map((as) => as.data),
        topic: AllQuestions4[curPage].question,
                  ...
                  {resultTie ? (
                    ...
                    <div className="mobile:w-full">
                      ...
                      <div className={!showItem ? 'block' : 'hidden'}>
                        {AllQuestions4.map((content, index) => (
                          <div
